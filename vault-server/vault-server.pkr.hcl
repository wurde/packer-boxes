...
source "amazon-ebs" "vault-server" {
  ...
  ami_name        = "vault-server-amazon-ebs-${local.timestamp}"
  ami_description = "An Amazon Linux 2 AMI for deploying a Vault server."
...
source "googlecompute" "vault-server" {
  ...
  image_name        = "vault-server-googlecompute-${local.timestamp}"
  image_description = "A Minimal Ubuntu Image for deploying a Vault server."
