...
source "amazon-ebs" "nomad-client" {
  ...
  ami_name        = "nomad-client-amazon-ebs-${local.timestamp}"
  ami_description = "An Amazon Linux 2 AMI for deploying a Nomad client."
...
source "googlecompute" "nomad-client" {
  ...
  image_name        = "nomad-client-googlecompute-${local.timestamp}"
  image_description = "A Minimal Ubuntu Image for deploying a Nomad client."
...
build {
  ...
  # /tmp directly.
  provisioner "file" {
    source      = "./nomad-client/scripts/"
    destination = "/tmp"
  }

  # Run the local setup script.
  provisioner "shell-local" {
    inline           = ["sh ./nomad-client/scripts/setup-local.sh"]
